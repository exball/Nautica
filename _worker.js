...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    // Build URI
    const uri = new URL(`tjn://${hostName}`);
    ...
    for (let i = startIndex; i < startIndex + PROXY_PER_PAGE; i++) {
      ...
      for (const port of PORTS) {
        ...
        for (const protocol of PROTOCOLS) {
          ...
          uri.searchParams.set("sni", port == 80 && protocol == "vls" ? "" : hostName);
    ...
    return `An error occurred while generating the vls configurations. ${error}`;
  }
}
      ...
      if (url.pathname.startsWith("/sub")) {
        ...
        if (apiPath.startsWith("/domains")) {
          ...
          for (const proxy of proxyList) {
            const uri = new URL(`tjn://${fillerDomain}`);
            uri.searchParams.set("encryption", "none");
            uri.searchParams.set("type", "ws");
            ...
            for (const port of filterPort) {
              for (const protocol of filterVPN) {
                ...
                uri.searchParams.set("security", port == 443 ? "tls" : "none");
                uri.searchParams.set("sni", port == 80 && protocol == "vls" ? "" : APP_DOMAIN);
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          ...
          if (protocol === "tjn") {
            protocolHeader = parsetjnHeader(chunk);
          } else if (protocol === "vls") {
            protocolHeader = parsevlsHeader(chunk);
...
async function protocolSniffer(buffer) {
  if (buffer.byteLength >= 62) {
    const tjnDelimiter = new Uint8Array(buffer.slice(56, 60));
    if (tjnDelimiter[0] === 0x0d && tjnDelimiter[1] === 0x0a) {
      if (tjnDelimiter[2] === 0x01 || tjnDelimiter[2] === 0x03 || tjnDelimiter[2] === 0x7f) {
        if (tjnDelimiter[3] === 0x01 || tjnDelimiter[3] === 0x03 || tjnDelimiter[3] === 0x04) {
          return "tjn";
        }
      }
    }
  ...
  const vlsDelimiter = new Uint8Array(buffer.slice(1, 17));
  // Hanya mendukung UUID v4
  if (arrayBufferToHex(vlsDelimiter).match(/^[0-9a-f]{8}[0-9a-f]{4}4[0-9a-f]{3}[89ab][0-9a-f]{3}[0-9a-f]{12}$/i)) {
    return "vls";
  }
...
function parsevlsHeader(vlsBuffer) {
  const version = new Uint8Array(vlsBuffer.slice(0, 1));
  let isUDP = false;

  const optLength = new Uint8Array(vlsBuffer.slice(17, 18))[0];

  const cmd = new Uint8Array(vlsBuffer.slice(18 + optLength, 18 + optLength + 1))[0];
  if (cmd === 1) {
  } else if (cmd === 2) {
    isUDP = true;
  } else {
  ...
  const addressBuffer = new Uint8Array(vlsBuffer.slice(addressIndex, addressIndex + 1));
  ...
  switch (addressType) {
    case 1: // For IPv4
      addressLength = 4;
      addressValue = new Uint8Array(vlsBuffer.slice(addressValueIndex, addressValueIndex + addressLength)).join(".");
      break;
    case 2: // For Domain
      addressLength = new Uint8Array(vlsBuffer.slice(addressValueIndex, addressValueIndex + 1))[0];
      addressValueIndex += 1;
      addressValue = new TextDecoder().decode(vlsBuffer.slice(addressValueIndex, addressValueIndex + addressLength));
      break;
    case 3: // For IPv6
      addressLength = 16;
      const dataView = new DataView(vlsBuffer.slice(addressValueIndex, addressValueIndex + addressLength));
      const ipv6 = [];
      for (let i = 0; i < 8; i++) {
        ipv6.push(dataView.getUint16(i * 2).toString(16));
      }
  ...
  return {
    ...
    rawClientData: vlsBuffer.slice(addressValueIndex + addressLength),
    version: new Uint8Array([version[0], 0]),
    isUDP: isUDP,
  };
}

function parsetjnHeader(buffer) {
  ...
  buildProxyGroup() {
    ...
    for (let i = 0; i < this.proxies.length; i++) {
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        const indexName = ["tjn TLS", "vls TLS", "SS TLS", "tjn NTLS", "vls NTLS", "SS NTLS"];
